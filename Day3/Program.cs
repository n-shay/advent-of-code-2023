...
const string input2 = @".......................661.........................485..565.......344.......325.....................................841.....725.............
...
for (var i = 0; i < lines.Count; i++)
{
    ...
    foreach (Match match in regex.Matches(line))
    {
        ...
        var endIndex = match.Index + match.Value.Length - 1;
        ...
        if (i > 0)
        {
            ...
            if (endIndex < l2.Length - 1) len++;    
            var sub = l2.Substring(st, len);     

            Console.WriteLine($"Pre: {sub}");
            
            var sl = sub.Select((c, j) => new {c, j}).Where(a => !char.IsDigit(a.c) && a.c != '.').Select(a => a.j + st).ToArray();
            if (sl.Any()) 
            {
                isPart = true;
                ...
                foreach(var j in sl)
                {
                    if (l2[j] == '*')
                    {
                        if(!gears.ContainsKey((i-1, j))) gears[(i-1, j)] = new List<int>();
                        gears[(i-1, j)].Add(val);
                    }
                }
            }
        ...
        // check prefix 
        if (!isPart && startIndex > 0)
        {
            var j = startIndex - 1;
            var prefix = line[j];
            if (!char.IsDigit(prefix) && prefix != '.') 
            {
                isPart = true;

                if (prefix == '*')
                {
                    if(!gears.ContainsKey((i, j))) gears[(i, j)] = new List<int>();
                    gears[(i, j)].Add(val);
                }
            }
        }
        ...
        // check postfix
        if (!isPart && endIndex + 1 < line.Length)
        {
            var j = endIndex + 1;
            var postfix = line[j];
            if (!char.IsDigit(postfix) && postfix != '.') 
            {
                isPart = true;
        ...
        if (!isPart)
        {
            Console.WriteLine($"{val} is NOT part #.");
            continue;
        }

        Console.WriteLine($"{val} is part #.");
        list.Add(val);
    }
}
...
var gearRatio = gears.Where(kvp => kvp.Value.Count == 2).Select(kvp => kvp.Value[0] * kvp.Value[1]).Sum();
