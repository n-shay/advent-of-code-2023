...
while ((line = sr.ReadLine()) != null)
{
    ...
    var first_pair = dic.Keys.Select(k => new { i = line.IndexOf(k), k = k})
        .Where(a => a.i > -1)
        .MinBy(a => a.i);

    if(first_pair is not null)
    {
        var regex = new Regex(first_pair.k);
        firstdigit = regex.Replace(line, dic[first_pair.k], 1);
    }
    
    var last_pair = dic.Keys.Select(k => new { i = line.LastIndexOf(k), k = k})
        .Where(a => a.i > -1)
        .MaxBy(a => a.i);

    if(last_pair is not null)
    {
        var i = lastdigit.LastIndexOf(last_pair.k);
        
        lastdigit = line.Remove(i, last_pair.k.Length).Insert(i, dic[last_pair.k]);
    }

    var d = int.Parse(new string([firstdigit.First(char.IsDigit), lastdigit.Last(char.IsDigit)]));
