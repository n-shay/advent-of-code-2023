...
var input2 = @"Game 1: 2 blue, 3 red; 3 green, 3 blue, 6 red; 4 blue, 6 red; 2 green, 2 blue, 9 red; 2 red, 4 blue
...
var input1 = @"Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
while ((line = sr.ReadLine()) != null)
{
    ...
    var game = int.Parse(arr[0].Replace("Game ", ""));
    var possible = true;
    var minRed = 0;
    var minGreen = 0;
    var minBlue = 0;
    ...
    {
        var colorArr = arr[i].Trim().Split(',');

        foreach (var colorItem in colorArr)
        {
            var itemArr = colorItem.Trim().Split(' ');
            var count = int.Parse(itemArr[0]);
            var color = itemArr[1].Trim();
    ...
    if (possible) possibleGames.Add(game);

    var power = minRed * minGreen * minBlue;
    //Console.WriteLine($"Game {game}: Power {power}");
    powerTotal += power;
}
