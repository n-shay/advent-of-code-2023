...
const string INPUT1 = @"..F7.
.FJ|.
SJ.L7
|F--J
LJ...";

const string INPUT2 = @"L-|---|.F--F7F-.FF-F-7FL|.F7.FF|7FL7.JF7F7-F..FF-777-7F|-7.FF--F7FL-7-|7F|7--J.F-7-|-F77.F-FL-L.F--|.L77-J-7-FF-7F|---7F77F|7F-|-L-7-F7..FF-
...
const string INPUT3 = @"FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
Side[] SupportedSides(char current)
{
    switch (current)
    {
        case 'S':
...
char ConnectingPipe(Side side1, Side side2)
{
    var sides = new[] { side1, side2 };

    if (!new[] { Side.Left, Side.Right }.Except(sides).Any()) return '-';
    if (!new[] { Side.Top, Side.Bottom }.Except(sides).Any()) return '|';
    if (!new[] { Side.Right, Side.Bottom }.Except(sides).Any()) return 'F';
    if (!new[] { Side.Top, Side.Right }.Except(sides).Any()) return 'L';
    if (!new[] { Side.Top, Side.Left }.Except(sides).Any()) return 'J';
    if (!new[] { Side.Left, Side.Bottom }.Except(sides).Any()) return '7';

    throw new Exception("How did we get here!?");
}

char[] SupportedChars(Side side)
{
    switch (side)
    {
        case Side.Top:
        ...
        case Side.Left:
            return ['-', 'F', 'L', 'S'];
        default:
            throw new Exception("What side is that!?");
    }
}

Point[] FindNext(char[,] map, Point pos)
{
    var options = new List<Point>();

    var current = map[pos.X, pos.Y];

    var sides = SupportedSides(current);

    // top
    if (sides.Contains(Side.Top) && pos.Y > 0 && SupportedChars(Side.Top).Contains(map[pos.X, pos.Y - 1])) options.Add(new Point(pos.X, pos.Y - 1));

    // left
    if (sides.Contains(Side.Left) && pos.X > 0 && SupportedChars(Side.Left).Contains(map[pos.X - 1, pos.Y])) options.Add(new Point(pos.X - 1, pos.Y));

    // bottom 
    if (sides.Contains(Side.Bottom) && pos.Y < map.GetLength(1) - 1 && SupportedChars(Side.Bottom).Contains(map[pos.X, pos.Y + 1])) options.Add(new Point(pos.X, pos.Y + 1));

    // right
    if (sides.Contains(Side.Right) && pos.X < map.GetLength(0) - 1 && SupportedChars(Side.Right).Contains(map[pos.X + 1, pos.Y])) options.Add(new Point(pos.X + 1, pos.Y));

    return options.ToArray();
}

Side CalculateSide(Point point, Point other)
{
    if (point.X == other.X)
    {
        if (point.Y > other.Y) return Side.Top;
        return Side.Bottom;
    }

    if (point.X > other.X) return Side.Left;
    return Side.Right;
}

void Run(string input, bool calcArea)
{
    ...
    for (var x = 0; x < map.GetLength(0); x++)
    {
        for (var y = 0; y < map.GetLength(1); y++)
        {
            ...
            if (map[x, y] == 'S') steps.Add(new Point(x, y));
        }
    }
    ...
    {
        steps.Add(current);
        var next = FindNext(map, current);

        // replace S with its char
        if (previous == null)
        {
            var side1 = CalculateSide(current, next[0]);
            var side2 = CalculateSide(current, next[1]);

            map[current.X, current.Y] = ConnectingPipe(side1, side2);
        }

        if (previous == null || previous == next[1])
        {
            previous = current;
            current = next[0];
        }
    ...
    if (!calcArea)
    {
        Console.WriteLine($"Steps: {steps.Count / 2}");
    }

    if (calcArea)
    {
        var inner = new bool[map.GetLength(0), map.GetLength(1)];

        // clear any none pipe point
        for (var x = 0; x < inner.GetLength(0); x++)
        {
            for (var y = 0; y < inner.GetLength(1); y++)
            {
                var point = new Point(x, y);
                if (!steps.Contains(point)) map[x, y] = '.';
            }
        }

        // mark potential horizontal inners
        for (var y = 0; y < inner.GetLength(1); y++)
        {
            var sides = 0;
            Side? openPipe = null;
            ...
            for (var x = 0; x < inner.GetLength(0); x++)
            {
                ...
                switch (ch)
                {
                    ...
                    case 'L':
                        ...
                        break;
                    case 'F':
                        openPipe = Side.Bottom;
                        break;
                    case 'J':
                        if(openPipe == Side.Bottom) sides++;
                        openPipe = null;
                        break;
                    case '7':
                        if(openPipe == Side.Top) sides++;
                        openPipe = null;
                        break;
                    case '.':
                        if (sides % 2 == 1) inner[x, y] = true;
                        break;
                }
            }
        }

        Console.WriteLine($"Inner: {inner.Cast<bool>().Where(b => b).Count()}");
    }
}
